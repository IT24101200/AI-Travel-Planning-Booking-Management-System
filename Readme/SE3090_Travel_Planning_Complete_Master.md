...
**`TripRequest`** — Id (PK), CustomerId (FK), DestinationId (FK, nullable), RawRequestText, StartDate, EndDate, TravellerCount, BudgetCeiling, Currency, Status (Pending/Planning/AwaitingApproval/Completed/Failed), **RetryCount** (int, default 0 — the Coordinator's one retry), FailureReason, CreatedAt
...
**`AgentLog`** — Id (PK), TripRequestId (FK), AgentName, StepDescription (plain-English sentence), CreatedAt. Written directly by the Python agent service after each meaningful step — no JSON schema needed, just a readable trail.
...
11. ADRs written for: the flat transport model, the one-retry decision, the simplified Stripe integration, the soft-delete strategy
